...
const ShowUserAttendance = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      const response = await axios.get(`${serverURL}/api/attendance/dev-and-fin/get-all-attendances`);
  ...
  const applyFilters = () => {
    ...
    const filtered = attendanceData.filter((record) => {
      return (
        ...
        (!empRole || record.empRole.toLowerCase().includes(empRole.toLowerCase())) &&
  ...
  return (
    <div className="w-full mx-auto p-6 bg-gray-200 rounded-lg shadow-md">
      <h1 className="text-2xl font-bold text-center text-blue-600 mb-6">Attendance Records</h1>
          ...
          <tbody>
            {filteredData.map((record, index) => (
              <tr key={index} className="text-center">
                <td className="border border-gray-300 px-4 py-2">{record.empId}</td>
                <td className="border border-gray-300 px-4 py-2">{record.empName}</td>
                <td className="border border-gray-300 px-4 py-2">{record.empMobile}</td>
                <td className="border border-gray-300 px-4 py-2">{record.empEmail}</td>
                <td className="border border-gray-300 px-4 py-2">{record.empRole}</td>
