...
const ShowAllForms = ({ siteEngineerId }) => {
  ...
  const downloadAsExcel = () => {
    ...
    const cleanData = removeFields(allForms);
    exportToExcel(cleanData, "hdd.xlsx");
  };

  const removeFields = (data) => {
    const fieldsToRemove = ["_id", "__v", "siteEngId", "siteEngObjId", "status", "paymentReceipt", "workProgressPhotoOrVideo", "salesAmount","remarks"];
    ...
    <div className="w-full">
      ...
      {allForms && Array.isArray(allForms) ? (
        <>
          ...
          <div>
            <p className="text-lg text-blue-500 my-4 flex justify-self-center">
              Download as
            </p>
            <p className=" text-red-500 my-4 flex justify-self-center">Note: This data is very similar to database, so don't share it with anyone due to security concerns.</p>
          </div>

          <div className="flex justify-center items-center">
            {/* <button className="m-1 p-1 rounded-md w-12">
              <PdfIcon size={24} />
            </button> */}
            <button className="m-1 p-1 rounded-md w-12">
              <span
