...
const SiteEngineerAttendanceForm = ({ siteEng }) => {
  ...
  const handleWorkerSelect = (e) => {
    ...
    // Filter out already selected workerIds from dataToSend
    setDataToSend((prev) => {
      const newItems = selectedList.filter(
        (newWorker) =>
          !prev.some((existing) => existing.workerId === newWorker.workerId)
      );
      return [...prev, ...newItems];
    });

    setSelectedWorkers(selectedList); // optional: if you need it separately
    console.log(dataToSend);
  };

  const handleSubmit = async () => {
    if (Array.isArray(selectedWorkers) && selectedWorkers?.length === 0) {
    ...
    if (!latitude || !longitude) {
      alert(
        "Location is not available right now. please wait for location update."
      );
      return;
    }
    if (!address) {
      alert(
        "Address is not available right now. please wait for address update."
      );
      return;
    }
    ...
    const workersData = dataToSend?.map(({ workerId, workerName }) => ({
      workerId,
      workerName,
    }));
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-md shadow-md">
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        {selectedWorkers.length > 0 && (
          <div className="mb-4">
            <h3 className="text-lg font-semibold">Selected Manpowers</h3>
            {selectedWorkers.map(({ workerId, workerName }) => (
              ...
              >
                ...
                {!isCameraOpen[workerId] && !photos[workerId] && (
                  <button
                    onClick={() => startCameraForWorker(workerId)}
                    className="w-28 mt-4 text-white p-2 bg-blue-500 rounded-md"
                  >
                    Start Camera
                  </button>
                )}
                ...
                {photos[workerId] && (
                  <img
                    src={URL.createObjectURL(photos[workerId])}
        ...
        <div>
          <label className="font-bold" htmlFor="siteLocation">Site Location- </label>
